function modal(e) {
	...
	fetch(`http://127.0.0.1:8000/api/v1/titles/${e.target.id}`)
		...
		.then((data) => {
			...
			// Vote
			modalNode.querySelector("#modalVote").innerHTML = data.votes;
			// duration in hours and minutes
			const hours = Math.floor(data.duration / 60);
			const minutes = data.duration % 60;
			modalNode.querySelector(
				"#modalDuration"
			).innerHTML = `<i class="fa-solid fa-clock"></i> ${hours}h ${minutes}min`;
			// Time
			modalNode.querySelector(
				"#modalTime"
			).innerHTML = `<i class="fa-solid fa-calendar-days"></i> ${data.date_published}`;
			// Genre
			...
			modalNode.querySelector(
				"#modalDescription"
			).innerHTML = `Description: ${data.description}`;
			// Actors
